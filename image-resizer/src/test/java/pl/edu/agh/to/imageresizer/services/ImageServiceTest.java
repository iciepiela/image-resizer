...
public class ImageServiceTest {
    ...
    public void shouldReturnAllResizedImagesForSessionKey() {
        ...
        List<ResizedImage> resizedImages = List.of(
                new ResizedImage("key", "name", "base64", sessionKey, 200, 200),
                new ResizedImage("key2", "name2", "base64_2", sessionKey, 200, 200),
                new ResizedImage("key3", "name3", "base64_3", sessionKey, 200, 200));
        List<ResizedImage> damagedImages = List.of(
                new ResizedImage("key4", "name", "ERROR", "sessionKey", 0, 0),
                new ResizedImage("key5", "name2", "ERROR", "sessionKey_2", 0, 0));
        ...
        Mockito.when(resizedImageRepository.findResizedImagesBySessionKeyAndWidthAndHeight(sessionKey, 200, 200))
                .thenReturn(Flux.fromIterable(resizedImages));
        Mockito.when(resizedImageRepository.findResizedImagesBySessionKeyAndWidthAndHeight(sessionKey, 0, 0))
                .thenReturn(Flux.fromIterable(damagedImages));
        ...
        ImageService imageService = new ImageService(originalImageRepository, resizedImageRepository, imageResizer);
        // when and then
        StepVerifier.create(imageService.getResizedImagesForSessionKey(sessionKey,ImageSize.MEDIUM))
                .expectNextMatches(image -> resizedImages.get(0).equals(image))
                .expectNextMatches(image -> resizedImages.get(1).equals(image))
                .expectNextMatches(image -> resizedImages.get(2).equals(image))
                .expectNextMatches(image -> damagedImages.get(0).equals(image))
    ...
    public void shouldReturnAllResizedImagesByImageKey() {
        ...
        Mockito.when(resizedImageRepository.findResizedImagesByImageKeyAndWidthAndHeight(sessionKey, 200, 200))
                .thenReturn(Flux.fromIterable(resizedImages));
        Mockito.when(resizedImageRepository.findResizedImagesByImageKeyAndWidthAndHeight(sessionKey, 0, 0))
                .thenReturn(Flux.fromIterable(damagedImages));
        ...
        ImageService imageService = new ImageService(originalImageRepository, resizedImageRepository, imageResizer);
        // when and then
        StepVerifier.create(imageService.getResizedImagesByImageKey(sessionKey,ImageSize.MEDIUM))
                .expectNextMatches(image -> resizedImages.get(0).equals(image))
                .expectNextMatches(image -> resizedImages.get(1).equals(image))
                .expectNextMatches(image -> resizedImages.get(2).equals(image))
                .expectNextMatches(image -> damagedImages.get(0).equals(image))
...
//
//        ImageDto imageDto1 = new ImageDto("key", "name", "base64", 225, 225);
//        OriginalImage originalImage1 = new OriginalImage("name", "base64", 255, 255);
//
//        ResizedImage resizedImage1 = new ResizedImage("key", "name", "base64", sessionKey, 200, 200);
//        resizedImage1.setOriginalImageId(1L);
//
...
//        ImageResizer imageResizer = Mockito.mock(ImageResizer.class);
//
//        Mockito.when(imageResizer.resize(imageDto1, sessionKey)).thenReturn(Mono.error(new RuntimeException()));
//
//        ImageService imageService = new ImageService(originalImageRepository, resizedImageRepository, imageResizer);
//
//        // when and then
